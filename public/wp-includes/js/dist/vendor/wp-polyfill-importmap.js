...
(function () {
  ...
  if (!shimMode) {
    if (document.querySelectorAll('script[type=modules-shim],script[type=importmap-shim],link[rel=modulepreload-shim]').length) {
      shimMode = true;
    }
    else {
      let seenScript = false;
      for (const script of document.querySelectorAll('script[type=modules],script[type=importmap]')) {
        if (!seenScript) {
    ...
    s.addEventListener('load', () => {
      if (!(supportsDynamicImport = !!(dynamicImport = self._d))) {
        ...
        dynamicImport = (url, opts) => new Promise((resolve, reject) => {
          ...
          if (nonce)
            s.setAttribute('nonce', nonce);
          // Safari is unique in supporting modules script error events
          ...
          for (const mutation of mutations) {
            ...
            for (const node of mutation.addedNodes) {
              if (node.tagName === 'SCRIPT') {
                if (node.type === (shimMode ? 'modules-shim' : 'module'))
  ...
  function processScriptsAndPreloads (mapsOnly = false) {
    ...
    if (!mapsOnly)
      for (const script of document.querySelectorAll(shimMode ? 'script[type=modules-shim]' : 'script[type=modules]'))
  ...
  // this should always trigger because we assume es-modules-shims is itself a domcontentloaded requirement
  if (hasDocument) {
  ...
  function processScript (script, ready = readyStateCompleteCnt > 0) {
    ...
    const loadPromise = topLevelLoad(script.src || baseUrl, getFetchOpts(script), !script.src && script.innerHTML, !shimMode, isBlockingReadyScript && lastStaticLoadPromise)
      .then(() => {
        // if the type of the script tag "modules-shim", browser does not dispatch a "load" event
